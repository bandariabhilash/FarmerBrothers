...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["SCF"] != null && Request.QueryString["TechID"] != null && Request.QueryString["IsBillable"] != null)
            {
                ...
                String CallTypeIDStr = string.Empty;
                using (SqlCommand woQry = new SqlCommand("Select * from workorder where WorkorderID = " + SCF, con))
                {
                    ...
                    if (wodr.Read())
                    {
                        ...
                        CallTypeIDStr = wodr["WorkorderCalltypeid"].ToString();
                    }
                    wodr.Close();
                }
                ...
                SqlCommand cmd11 = new SqlCommand("Select * from [Symptom] where active = 1 order by SymptomID", con);
                SqlDataReader sdr11 = cmd11.ExecuteReader();
                SymptomCode.DataSource = sdr11;
                SymptomCode.DataTextField = "Description";
                SymptomCode.DataValueField = "SymptomID";
                SymptomCode.DataBind();
                SymptomCode.Items.Insert(0, new ListItem("", "0"));
                sdr11.Close();

                SqlCommand cmd10 = new SqlCommand("Select * from [SystemInfo] where active = 1 order by SystemId", con);
                SqlDataReader sdr10 = cmd10.ExecuteReader();
                SystemCode.DataSource = sdr10;
                SystemCode.DataTextField = "Description";
                SystemCode.DataValueField = "SystemId";
                SystemCode.DataBind();
                SystemCode.Items.Insert(0, new ListItem("", "0"));
                sdr10.Close();
                ...
                wrntyDD.DataBind();
                wrntyDD.Items.Add(new ListItem("", ""));
                wrntyDD.Items.Add(new ListItem("YES", "YES"));
                wrntyDD.Items.Add(new ListItem("NO", "NO"));

                wrntyForDD.DataBind();
                wrntyForDD.Items.Add(new ListItem("", ""));
                wrntyForDD.Items.Add(new ListItem("Just Parts", "Just Parts"));
                wrntyForDD.Items.Add(new ListItem("Parts and Labor", "Parts and Labor"));

                followupDD.DataBind();
                followupDD.Items.Add(new ListItem("", ""));
                followupDD.Items.Add(new ListItem("YES", "YES"));
                followupDD.Items.Add(new ListItem("NO", "NO"));

                operationalDD.DataBind();
                operationalDD.Items.Add(new ListItem("", ""));
                operationalDD.Items.Add(new ListItem("YES", "YES"));
                operationalDD.Items.Add(new ListItem("NO", "NO"));

                using (SqlCommand cmd13 = new SqlCommand("Select * from [dbo].[ERF] where convert(varchar(10), getdate(), 102) > convert(varchar(10), OriginalRequestedDate, 102) and WorkorderID = " + SCF, con))
                {
                    SqlDataReader dr13 = cmd13.ExecuteReader();
                    if (dr13.Read())
                    {
                        dr13.Close();
                        if (CallTypeIDStr == "1300")
                        {
                            calltypeID.Value = CallTypeIDStr;
                            ReasonCode.Visible = true;
                            SqlCommand cmd12 = new SqlCommand("select FBStatusID,FBStatus from AllFBStatus where StatusFor='ReScheduleReasonCode' order by FBStatus asc", con);
                            SqlDataReader sdr12 = cmd12.ExecuteReader();
                            RCode.DataSource = sdr12;
                            RCode.DataTextField = "FBStatus";
                            RCode.DataValueField = "FBStatusID";
                            RCode.DataBind();
                            RCode.Items.Insert(0, new ListItem("", "0"));
                            sdr4.Close();
                        }
                    }
                }
    ...
    public static string GetItems(string SID)
    {
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            using (SqlCommand cmd1 = new SqlCommand())
            {
                ...
                cmd1.CommandText = "Select SKUID, EntryNumber, ItemNo, VendorNo, Description, Supplier from FBClosureParts where SKUactive = 1 and SKUID > 0 and (EntryNumber = @SearchText  or ItemNo = @SearchText or VendorNo = @SearchText)";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        String SymptomCode1 = SymptomCode.Text.Replace("'", "''").ToString();
        String SystemCode1 = SystemCode.Text.Replace("'", "''").ToString();
        ...
        String temp = Temperature.Text.Replace("'", "''").ToString();
        ...
        String ReasonCode = "";
        if (calltypeID.Value == "1300")
        {
            ReasonCode = RCode.SelectedValue.ToString();
        }
        ...
        try
        {
            ...
            String sNewPass = Eventid.Length == 5 ? (Eventid.Substring(0, 3) + sGenPwd(2) + Eventid.Substring(Eventid.Length - 2)) : Eventid.Substring(0, 3) + sGenPwd(2) + Eventid.Substring(Eventid.Length - 6);
            SqlCommand cmd = new SqlCommand("INSERT INTO TMP_BlackBerry_SCFAssetInfo(WorkorderID, TechID, ServiceCode, SymptomCode, SystemCode, CompletionCode, EquipTypeCode, VendorCode, Model, SerialNo, ReasonCode, Notes, CloseCall, ClosureConfirmationNo, Weight, Ratio, Temperature) values ('" + Eventid + "', '" + Techid + "', '" + CallType1 + "', '" + SymptomCode1 + "', '" + SystemCode1 + "', '" + CompletionCode1 + "', '" + EquipType1 + "', '" + Vendor1 + "', '" + model1 + "', '" + serial1 + "', '" + ReasonCode + "', '" + EventNotes1 + "',0, '" + sNewPass + "', '" + weight + "','"+ratio+"','"+temp+"')", con);
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        if (serviceBillableChk.Checked == true)
        {
            string transactionId = GetTransactionId(Eventid);
            if(string.IsNullOrEmpty(transactionId))
            {
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please process the Credit Card from Email Link, before Closing the Event')", true);
                return;
            }
        }
        ...
        string Customersig = hfSign1.Value;
        string Techniciansig = hfSign2.Value;

        String stateOfEqp = statEqpTxt.Text.Replace("'", "''").ToString();
        String serviceDelayed = srvcDlyTxt.Text.Replace("'", "''").ToString();
        String troubleShootStps = trblShtStpsTxt.Text.Replace("'", "''").ToString();

        String isUnderWarrenty = wrntyDD.Text.Replace("'", "''").ToString();
        String warrentyFor = string.Empty;
        if (isUnderWarrenty.ToLower() == "yes")
        {
            warrentyFor = wrntyForDD.Text.Replace("'", "''").ToString();
        }

        String followUpNeeded = followupDD.Text.Replace("'", "''").ToString();
        String followupSteps = string.Empty;
        if (followUpNeeded.ToLower() == "yes")
        {
            followupSteps = followupTxt.Text.Replace("'", "''").ToString();
        }

        String isOperational = operationalDD.Text.Replace("'", "''").ToString();

        String reviewedBy = ReviewedByTxt.Text.Replace("'", "''").ToString();
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO TMP_BlackBerry_SCFInvoiceInfo(WorkorderID, TechID, StartDateTime, ArrivalDateTime, CompletionDateTime,SignedBy,InvoiceNo,WaterTested,HardnessRating,CustomerSign,TechnicianSign, StateofEquipment, ServiceDelayReason, TroubleshootSteps, FollowupComments, ReviewedBy, IsUnderWarrenty, WarrentyFor, AdditionalFollowupReq, IsOperational) values('" + Eventid + "', '" + Techid + "', '" + Sdate.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + Adate.ToString("yyyy-MM-dd HH:mm:ss") + "', '" + Cdate.ToString("yyyy-MM-dd HH:mm:ss") + "','" + SignedBy1 + "','" + InvNo1 + "','"+WaterTested+ "','" + waterHardness + "','" + Customersig + "','" + Techniciansig + "','"+stateOfEqp+"','"+serviceDelayed+"','"+troubleShootStps+"','"+followupSteps+"','"+reviewedBy+"','"+isUnderWarrenty+"','"+warrentyFor+"','"+followUpNeeded+"','"+isOperational+"')", con);
    ...
    protected void ClearData()
    {
        ...
        WeightTxt.Text = "";
        RatioTxt.Text = "";
        SystemCode.SelectedIndex = -1;
        SymptomCode.SelectedIndex = -1;
        Temperature.Text = "";
        RCode.SelectedIndex = -1;
    }
    ...
    public static string GetTransactionId(string EventId)
    {
        string transId = "";
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            using (SqlCommand cmd1 = new SqlCommand())
            {
                cmd1.CommandText = "Select FinalTransactionId from Workorder where workorderid=" + EventId;
                cmd1.Connection = conn;
                conn.Open();
                using (SqlDataReader sdr1 = cmd1.ExecuteReader())
                {
                    if (sdr1.Read())
                    {
                        transId = sdr1["FinalTransactionId"] != DBNull.Value ? sdr1["FinalTransactionId"].ToString() : "";
                    }
                    sdr1.Close();
                }
                conn.Close();
