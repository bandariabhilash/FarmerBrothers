    ...
    public class WorkorderRepository : IWorkorderRepository
    {
        ...
        public ResultResponse<ERFResponseClass> SaveERFWorkorder(WorkorderRequestModel RequestData, int userId, string userName)        
        {
            ...
            using (var _context = new FBContext())
            {
                ...
                if (RequestData.ERFId != null && RequestData.ERFId > 0)
                {
                    WorkOrder erfWO = _context.WorkOrders.Where(w => w.WorkorderErfid == RequestData.ERFId.ToString()).FirstOrDefault();
                    ...
                    if(erfWO != null)
                    {
                        ...
                        result.Data.WorkorderId = Convert.ToInt32(erfWO.WorkorderId);
                        result.Data.ERFId = Convert.ToInt32(erfWO.WorkorderErfid);
                ...
                using (var transaction = _context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(RequestData.HoursOfOperation))
                        {
                            errorMessage += "| Hours Of Operation";
                            isvalid = false;
                        }
                        ...
                        workorderModel.WorkOrder.WorkorderContactPhone = RequestData.MainContactNum;
                        workorderModel.WorkOrder.HoursOfOperation = RequestData.HoursOfOperation == null ? "N/A" : RequestData.HoursOfOperation;
                        ...
                        workorderModel.Erf.DateErfreceived = string.IsNullOrEmpty(RequestData.SubmissionDate) ? null : Convert.ToDateTime(RequestData.SubmissionDate);
                        workorderModel.Erf.OriginalRequestedDate = string.IsNullOrEmpty(RequestData.InstallDate) ? null : Convert.ToDateTime(RequestData.InstallDate);
                        workorderModel.Erf.InstallLocation = string.IsNullOrEmpty(RequestData.InstallLocation) ? "" : RequestData.InstallLocation;
                        workorderModel.Erf.SiteReady = string.IsNullOrEmpty(RequestData.SiteReady) ? "" : RequestData.SiteReady;
        ...
        public ResultResponse<ERFResponseClass> SaveWorkorderData(WorkorderManagementModel workorderManagement, int userId, string userName, FBContext _context)
        {
            ...
            try
            {
                ...
                //string erfComment = "Submission Date : " + workorderManagement.Erf.DateErfreceived + "\r\n" +
                //                                  "Install Date : " + workorderManagement.Erf.OriginalRequestedDate + "\r\n" +
                //                                  "Install Location : " + workorderManagement.Erf.InstallLocation + "\r\n" +
                //                                  "Site Ready : " + workorderManagement.Erf.SiteReady + "\r\n" +
                //                                  "Hours Of Operation : " + workorderManagement.WorkOrder.HoursOfOperation;

                string erfComment = "";
                if(workorderManagement.Erf.DateErfreceived != null)
                {
                    erfComment += "Submission Date : " + workorderManagement.Erf.DateErfreceived + "\r\n";
                }
                if (workorderManagement.Erf.OriginalRequestedDate != null)
                {
                    erfComment += "Install Date : " + workorderManagement.Erf.OriginalRequestedDate + "\r\n";
                }
                if (!string.IsNullOrEmpty(workorderManagement.Erf.InstallLocation))
                {
                    erfComment += "Install Location : " + workorderManagement.Erf.InstallLocation + "\r\n";
                }
                if (!string.IsNullOrEmpty(workorderManagement.Erf.SiteReady))
                {
                    erfComment += "Site Ready : " + workorderManagement.Erf.SiteReady + "\r\n";
                }
                if (!string.IsNullOrEmpty(workorderManagement.WorkOrder.HoursOfOperation))
                {
                    erfComment += "Hours Of Operation : " + workorderManagement.WorkOrder.HoursOfOperation + "\r\n";
                }


                NotesHistory erfcommentsHistory = new NotesHistory()
                {
                    AutomaticNotes = 0,
                    EntryDate = currentTime,
                    Notes = erfComment,
                ...
                _context.NotesHistories.Add(erfcommentsHistory);
