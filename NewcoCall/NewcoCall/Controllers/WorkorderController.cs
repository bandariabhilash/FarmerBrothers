...
namespace NewcoCall.Controllers
{
    public class WorkorderController : Controller
    {
        ...
        public JsonResult SaveWorkorder(CustomerServiceModel CustomerDetails)
        {
            ...
            if (isValid == true)
            {
                using (NewcoEntity newcoEntity = new NewcoEntity())
                {
                    ...
                    if (returnValue == 1)
                    {   
                        message = "Work order Created Successfully " + workOrder.WorkorderID;                        
                    }
        ...
        public int WorkOrderSave(CustomerModel customerdata, NewcoEntity newcoEntity, out WorkOrder workOrder, out string message)
        {
            ...
            workOrder.WorkorderCalltypeid = WOTyp.CallTypeID;
            workOrder.WorkorderCalltypeDesc = WOTyp.Description;
        ...
        public JsonResult GetServiceQuote_OLD(string ZipCode, string State, string ServiceType)
        {
            JsonResult jsonResult = new JsonResult();
            ...
            using (NewcoEntity newcoEntity = new NewcoEntity())
            {
                ...
                if (techHView != null)
                {
                    ...
                    WorkorderType currentServiceRate = newcoEntity.WorkorderTypes.Where(w => w.ViewIn.ToLower() == "newco" && w.Active == 1 && w.CallTypeID.ToString() == ServiceType).FirstOrDefault();
                    decimal laborRateDefined = 0;
                    if (currentServiceRate != null)
                    {
                        laborRateDefined = Convert.ToDecimal(currentServiceRate.cost);
                    }
        ...
        public JsonResult GetServiceQuote(string ServiceType, string ParentId)
        {
            JsonResult jsonResult = new JsonResult();
            ServiceQuote quote = new ServiceQuote();
            int flag = 0;
            ...
            using (NewcoEntity newcoEntity = new NewcoEntity())
            {
                ...
                if (currentServiceRate != null)
                {
                    if (ParentId == "9001256")
                    {
                        quote.TotalServiceQuote = currentServiceRate.cost != null ? currentServiceRate.cost.ToString() : "0";
                    }
                    else
                    {
                        quote.TotalServiceQuote = currentServiceRate.AlternateCost != null ? currentServiceRate.AlternateCost.ToString() : "0";
                    }
                ...
                {
                    message = "Problem getting the ServiceCost!";
                }
            }
