...
namespace NewcoCall.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            List<WorkorderType> WOTypeList = newcoEntity.WorkorderTypes.Where(w => w.ViewIn.ToLower() == "newco" && w.Active == 1).OrderBy(o => o.Description).ToList();
        ...
        public JsonResult GetCustomerDetails(string CustomerNumber)
        {
            ...
            Contact contact = newcoEntity.Contacts.Where(c => c.LongAddressNumber == CustomerNumber.ToString() && newcoParentIdsList.Contains(c.PricingParentID)).FirstOrDefault();
            //if (contact == null &&  (int.Parse(CustomerNumber.ToString()).GetType().Equals(typeof(int))))
            if (contact == null)
            {
                //int contactid = Convert.ToInt32(CustomerNumber);
                contact = newcoEntity.Contacts.Where(c => c.ContactID.ToString() == CustomerNumber.ToString() && newcoParentIdsList.Contains(c.PricingParentID)).FirstOrDefault();
            }
            ...
            {
                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, message = "Invalid Newco Customer !" };
            }
