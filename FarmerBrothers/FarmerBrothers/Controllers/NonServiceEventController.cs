...
namespace FarmerBrothers.Controllers
{
    public class NonServiceEventController : BaseController
    {
        ...
        */
        ...
        public bool SendMail_old(int WorkOrderID, FarmerBrothersEntities FarmerBrothersEntitites)
        {
        ...
        public bool SendMail(int WorkOrderID, FarmerBrothersEntities FarmerBrothersEntitites)
        {
                ...
                && callReason != null && callReason.Description.ToLower() != "dropped call")
            {
                ...
                if (!mailToUserIds.ContainsKey(emailIds[0])) mailToUserIds.Add(new KeyValuePair<string, string>(emailIds[0], "CustomerService"));
                if (!mailToUserIds.ContainsKey(emailIds[1])) mailToUserIds.Add(new KeyValuePair<string, string>(emailIds[1], "Kim"));
            ...
            else
            {
                ...
                {
                    mailTo += ConfigurationManager.AppSettings["KimEmailId"].ToString();
                    mailTo += ";";

                    if (!mailToUserIds.ContainsKey(ConfigurationManager.AppSettings["KimEmailId"].ToString())) mailToUserIds.Add(new KeyValuePair<string, string>(ConfigurationManager.AppSettings["KimEmailId"].ToString(), "Kim"));
                    mailToName += "Kim - " + ConfigurationManager.AppSettings["KimEmailId"].ToString() + ";    ";
            ...
            if (callReason != null && callReason.Description.ToLower() == "complaint")
            {
                mailTo = "";
                mailToName = "";
                if (callReason.EmailSalesManager == true && !string.IsNullOrEmpty(csd.SalesMmanagerEmail))
                ...
                mailTo += ConfigurationManager.AppSettings["LisaJensenEmailId"].ToString();
                mailToName += "Lisa Jensen - " + ConfigurationManager.AppSettings["LisaJensenEmailId"].ToString() + ";    ";
            ...
            string subject = "Call Reason: " + callReason.SourceCode + " - " + CallReasonDesc + ", Customer Service Event#: " + WorkOrderID;
            if (Convert.ToBoolean(ConfigurationManager.AppSettings["UseTestMails"]))
            {
                mailTo = ConfigurationManager.AppSettings["TestEmail"];
            }
            ...
            try
            {
                ...
                eu.SendEmail(fromAddress, mailTo, bcc, subject, salesEmailBody.ToString());
            }
            catch
            {
                result = false;
