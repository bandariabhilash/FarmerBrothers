            ...
            if (workOrder != null)
            {
                ...
                if (updatedStatus == true)
                {
                    ...
                    if (workOrder.WorkorderDetails != null && workOrder.WorkorderDetails.Count > 0)
                    {
                        ...
                        if (details != null)
                        {
                            details.ResponsibleTechid = techHierarchyView.TechID;
                        ...
                        {
                            WorkorderDetail dtls = new WorkorderDetail();
                            dtls.WorkorderID = workOrder.WorkorderID;
                            dtls.ResponsibleTechid = techHierarchyView.TechID;
                            dtls.ResponsibleTechName = techHierarchyView.PreferredProvider;
                            TECH_HIERARCHY technicianAddress = dc.GetCustomerByTechId(techId.ToString());
                            string fromAddress = workOrder.CustomerAddress + " " + workOrder.CustomerCity + " " + workOrder.CustomerState + " " + workOrder.CustomerZipCode;
                            string toAddress = string.Empty;
                            if (!string.IsNullOrEmpty(technicianAddress.Address1))
                            ...
                            dtls.Mileage = DispatchResponseController.GetDistance(fromAddress, toAddress);
                        }
                    }
