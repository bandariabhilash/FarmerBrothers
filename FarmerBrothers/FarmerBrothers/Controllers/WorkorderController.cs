    ...
    public class WorkorderController : BaseController
    {
            ...
            try
            {
                ...
                if (workOrderManagementModel.WorkOrder != null)
                {
                    ...
                    if (ws != null)
                    {
                        ...
                        workOrderManagementModel.RedirectUrl = string.Format("{0}{1}&encrypt=yes", Redircturl, new Encrypt_Decrypt().Encrypt("workOrderId=" + workOrderManagementModel.WorkOrder.WorkorderID + "&techId=" + ws.Techid + "&response=5&isResponsible=true"));
                ...
                NonFBCustomer nonFbCust = FarmerBrothersEntitites.NonFBCustomers.Where(c => c.NonFBCustomerId == workOrderManagementModel.Customer.NonFBCustomerNumber).FirstOrDefault();
                if (nonFbCust == null)
                {
                    workOrderManagementModel.isCustomerPoRequired = false;
                }
                else
                {
                    workOrderManagementModel.isCustomerPoRequired = nonFbCust.IsCustomerPORequired == null ? false : nonFbCust.IsCustomerPORequired;
                }
        ...
        public JsonResult GetServiceQuoteDetailsWithParts(UpdateServiceQuoteModel serviceQuoteObj)
        {
            ...
            try
            {
                ...
                decimal? SkuTotal = 0;
                if (serviceQuoteObj.SkuList != null)
                {
                    //decimal? SkuTotal = serviceQuoteObj.SkuList.Where(w => w.PartReplenish == false).Select(s => s.Quantity * s.StandardCost).Sum();
                    SkuTotal = serviceQuoteObj.SkuList.Select(s => s.Quantity * s.StandardCost).Sum(); //Removed PartReplenish check as per the request from Mike on Feb 12, 2025 (RE: service quote in work order and call closure module)
                }
        ...
        private IList<WorkorderSearchResultModel> GetWorkOrderDataUsingQuery(WorkorderSearchModel workorderSearchModel)
        {
            ...
            woSearchSelectQuery.Append(@"select w.WorkorderID ,wt.description, w.WorkorderCallstatus, st.fbstatus, c.ServiceLevelCode,w.WorkorderEntryDate,w.AppointmentDate,
                                 ...
                                 ws.ScheduleDate,ws.ModifiedScheduleDate,ws.ServiceCenterID,w.CustomerPO,w.NTE, w.ThirdPartyPO ");
            ...
            woSearchJoinQuery.Append(@" from workorder w WITH (NOLOCK)
                                        ...
                                        left join WorkorderType wt WITH (NOLOCK) on w.WorkorderCalltypeid = wt.calltypeid
                                        left join AllFBStatus st WITH (NOLOCK) on  w.PriorityCode =st.fbstatusid
        ...
        */
                ...
                case WorkOrderManagementSubmitType.SAVE:
                    {
                        ...
                        if (string.Compare(workorderManagement.WorkOrder.WorkorderCallstatus, "Closed", true) != 0
                            ...
                            && workorderManagement.WorkOrder.WorkorderCalltypeDesc != "Parts Request"
                        ...
                        //&& workorderManagement.Closure.SpecialClosure.Contains("No Service Required") == false)
                        {
                            ...
                            && workorderManagement.WorkOrder.WorkorderCalltypeid != 1810
                            && workorderManagement.WorkOrder.WorkorderCalltypeid != 1820
                            &&*/ workorderManagement.IsNewPartsOrder != true && workorderManagement.WorkOrder.WorkorderCalltypeDesc != "Parts Request"
                            && workorderManagement.Closure.SpecialClosure.Contains("No Service Required") == false
                            && workorderManagement.Closure.SpecialClosure.Contains("Cancellation") == false)
                            {
                                ...
                                if (workorderManagement.Customer.IsNonFBCustomer)
                                {
                                    NonFBCustomer nonfbCust = FarmerBrothersEntitites.NonFBCustomers.Where(c => c.NonFBCustomerId == workorderManagement.Customer.NonFBCustomerNumber && c.IsCustomerPORequired == true).FirstOrDefault();
                                    if (nonfbCust != null)
                                    {
                                        if (workorderManagement.WorkOrder.WorkorderCallstatus.ToLower() != "closed" && string.IsNullOrEmpty(workorderManagement.WorkOrder.CustomerPO))
                                        {
                                            message = @"|CustomerPO required!";
                                            returnValue = -1;
                                            isValid = false;
                                        }
                                    }
        ...
        public int WorkOrderSave(WorkorderManagementModel workorderManagement, WorkOrderManagementSubmitType operation, out WorkOrder workOrder, out string message, bool isAutoGenWO = false)
        {
            ...
            if (operation == WorkOrderManagementSubmitType.CREATEWORKORDER)
            {
                ...
                workOrder.WorkorderEntryUserid = isAutoGenWO ? 99999 : System.Web.HttpContext.Current.Session["UserId"] != null ? Convert.ToInt32(System.Web.HttpContext.Current.Session["UserId"]) : 1234;
                ...
                if (isValidErf == true)
                {
                    ...
                    if (workorderManagement.WorkOrder.WorkorderID > 0)
                    {
                        ...
                        if (string.Compare(workorderManagement.WorkOrder.WorkorderCallstatus, "Closed", true) != 0)
                        {
                            ...
                            if (workorderManagement.IsNewPartsOrder == false && workorderManagement.WorkOrder.WorkorderCalltypeDesc != "Parts Request" && workorderManagement.WorkOrderEquipmentsRequested != null)
                            {
        ...
        public int OverTimeRequest(WorkorderManagementModel workOrderManagementModel, out WorkOrder workOrder)
        {
            int returnValue = -1;
            ...
            if (workOrder != null)
            {
                ...
                if (returnValue > 0)
                {
                    ...
                    Contact cnct = FarmerBrothersEntitites.Contacts.Where(c => c.ContactID.ToString() == workOrderManagementModel.Customer.CustomerId).FirstOrDefault();
                    if (cnct != null)
                    {
                        int esm = cnct.FSMJDE == null ? 0 : Convert.ToInt32(cnct.FSMJDE);
                        ESMCCMRSMEscalation esmdetails = FarmerBrothersEntitites.ESMCCMRSMEscalations.Where(e => e.EDSMID == esm).FirstOrDefault();
                        string ESMEmail = "";
                        if (esmdetails != null)
                        {
                            ESMEmail = string.IsNullOrEmpty(esmdetails.ESMEmail) ? "" : esmdetails.ESMEmail;
                        }

                        SendWorkOrderMail(workOrder, subject.ToString(), ESMEmail.ToString(), ConfigurationManager.AppSettings["CustomerUpdateMailFromAddress"], null, MailType.INFO, false, null);
        ...
        public void CreateSpawnWorkOrder(WorkorderManagementModel workorderManagement, List<WorkorderEquipment> equipment, out string message)
        {
            ...
            foreach (int soluitonId in uniqueSolutionIds)
            {
                 ...
                 || soluitonId == 5191)
                {
                    ...
                    spawnWorkOrder.Estimate = null;
                    spawnWorkOrder.FinalEstimate = null;
                    spawnWorkOrder.ThirdPartyPO = null;
                    spawnWorkOrder.EstimateApprovedBy = null;
                    spawnWorkOrder.IsEstimateApproved = false;
                    if (workOrder.OriginalWorkorderid.HasValue)
                    {
                        spawnWorkOrder.OriginalWorkorderid = workOrder.OriginalWorkorderid;
                    }
                    ...
                    if (workOrder.WorkorderDetails.Count > 0)
                    {
                        ...
                        spawnWorkOrderDetail.InvoiceNo = "";
                        spawnWorkOrderDetail.SolutionId = soluitonId;
                        spawnWorkOrderDetail.StateofEquipment = null;
                        spawnWorkOrderDetail.ServiceDelayReason = null;
                        spawnWorkOrderDetail.TroubleshootSteps = null;
                        spawnWorkOrderDetail.FollowupComments = null;
                        spawnWorkOrderDetail.OperationalComments = null;
                        spawnWorkOrderDetail.ReviewedBy = null;
                        spawnWorkOrderDetail.IsUnderWarrenty = null;
                        spawnWorkOrderDetail.WarrentyFor = null;
                        spawnWorkOrderDetail.AdditionalFollowupReq = null;
                        spawnWorkOrderDetail.IsOperational = null;
                        spawnWorkOrderDetail.TechnicianSignatureDetails = null;
                        spawnWorkOrderDetail.CustomerSignatureDetails = null;
                        spawnWorkOrderDetail.CustomerSignatureBy = null;
                        spawnWorkOrderDetail.CustomerSignature = null;
                        spawnWorkOrderDetail.Mileage = null;
                        spawnWorkOrderDetail.TotalDissolvedSolids = null;
                ...
                if ((mailType == MailType.DISPATCH || mailType == MailType.SPAWN) && techId.HasValue)
                {
                    if (string.Compare(workOrder.WorkorderCallstatus, "Closed", true) != 0)
                    {
                        ...
                        if (mailType == MailType.DISPATCH)
                        {
                            ...
                            //salesEmailBody.Append("<a onclick = 'if (confirm('Delete selected item?')){return true;}else{event.stopPropagation(); event.preventDefault();};' href=\"" + string.Format("{0}{1}&encrypt=yes", url, new Encrypt_Decrypt().Encrypt("workOrderId=" + workOrder.WorkorderID + "&techId=" + techId.Value + "&response=0&isResponsible=" + isResponsible.ToString())) + "\">ACCEPT</a>");
                            salesEmailBody.Append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
                        }
        ...
        public StringBuilder GetEmailBodyWithLinks_New(WorkOrder workOrder, string subject, string toAddress, string fromAddress, int? techId, MailType mailType, bool isResponsible, string additionalMessage, string mailFrom = "", bool isFromEmailCloserLink = false, string SalesEmailAddress = "", string esmEmailAddress = "")
        {
            StringBuilder salesEmailBody = new StringBuilder();
        ...
        public int NotifySales(WorkorderManagementModel workOrderManagementModel, out WorkOrder workOrder)
        {
            int returnValue = -1;
            StringBuilder salesEmailBody = new StringBuilder();
            workOrder = FarmerBrothersEntitites.WorkOrders.Where(w => w.WorkorderID == workOrderManagementModel.WorkOrder.WorkorderID).FirstOrDefault();
            ...
            if (!string.IsNullOrEmpty(emailId))
            {
                try
                {
                    ...
                    else
                    {
                        if (Convert.ToBoolean(ConfigurationManager.AppSettings["UseTestMails"]))
                        {
                            emailId = ConfigurationManager.AppSettings["TestEmail"];
                        }
        ...
        public JsonResult CreateInvoice(string workOrderId)
        {
            try
            {
                ...
                {
                    WorkorderSchedule schedule = FarmerBrothersEntitites.WorkorderSchedules.Where(ws => ws.WorkorderID == workOrder_Id && ws.PrimaryTech >= 0 && (ws.AssignedStatus == "Accepted" || ws.AssignedStatus == "Scheduled")).FirstOrDefault();
...
ENTIRE AGREEMENT; ASSIGNMENT:&nbsp;<span style='font-weight:normal;text-align:justify;display: block;'>The terms and conditions set forth herein, or as changed or modified by a written agreement signed by Operator and FBC, shall constitute the entire contract between Operator and FBC with respect to the subject matter herein and shall supersede any additional or inconsistent terms and conditions contained in any proposals, invoices, orders, or any other documents or correspondence of Operator. For the avoidance of doubt, this Agreement does not apply to any goods(other than the equipment) that FBC may sell to Operator.All such goods shall be governed by FBC’s standard Terms and Conditions of Sale, which shall not be superseded by this Agreement.No changes or modifications to the terms and conditions set forth herein shall have any force or effect, unless otherwise agreed to in writing by Operator and FBC. Neither party may assign, delegate or otherwise transfer this Agreement, in whole or in part, without the prior written consent of the other party (not to be unreasonably withheld); provided, that FBC may assign this Agreement to any party controlling, controlled by or under common control with FBC or to any person acquiring all or substantially all of the assets or outstanding capital stock of FBC.Any attempted assignment, delegation, or other transfer of this Agreement in violation of this Section shall be null and void. An electronic image of this document and any signature or acknowledgement thereto will be considered an original (to the same extent as any paper or hard copy), including under evidentiary standards applicable to a proceeding between the parties hereto.[ The current version of this Agreement and any modifications or amendments supersede all prior versions of this Agreement.The most current version of this Agreement may be found at FBC’s website(www.farmerbros.com / ____) and is otherwise available upon request.]</span></td>
        ...
        public ActionResult WorkorderNSRClose(string sku)
        {
            WorkorderNSRCloseModel workorderModel = new WorkorderNSRCloseModel();
            return View(workorderModel);
        }

        public JsonResult CloseEventsNSR(string workOrderIdStr, bool isCSE)
        {
            //List<int> eventsList = workOrderIdStr.Split(',').Select(int.Parse).ToList();
            List<int> eventsList = workOrderIdStr.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToList();
            List<WorkorderNSRCloseResultModel> resultList = new List<WorkorderNSRCloseResultModel>();

            using (FarmerBrothersEntities entity = new FarmerBrothersEntities())
            {
                foreach (int wo in eventsList)
                {
                    WorkorderNSRCloseResultModel result = new WorkorderNSRCloseResultModel();

                    if (isCSE)
                    {
                        NonServiceworkorder cse = entity.NonServiceworkorders.Where(w => w.WorkOrderID == wo).FirstOrDefault();
                        if (cse == null)
                        {
                            result.WorkOrderId = wo;
                            result.WorkorderStatus = "";
                            result.CloseStatus = "Failed";
                            result.Message = "Invalid CSE Id";
                        }
                        else
                        {
                            try
                            {
                                DateTime CurrentTime = Utility.GetCurrentTime(cse.CustomerZipCode, entity);

                                cse.NonServiceEventStatus = "Closed";
                                cse.CloseDate = CurrentTime;
                                cse.ClosedBy = System.Web.HttpContext.Current.Session["UserId"] != null ? Convert.ToInt32(System.Web.HttpContext.Current.Session["UserId"]) : 1234;

                                NotesHistory notesHistory = new NotesHistory()
                                {
                                    AutomaticNotes = 1,
                                    EntryDate = CurrentTime,
                                    Notes = "CSE Closed as per the ESM",
                                    Userid = System.Web.HttpContext.Current.Session["UserId"] != null ? Convert.ToInt32(System.Web.HttpContext.Current.Session["UserId"]) : 1234,
                                    UserName = UserName,
                                    NonServiceWorkorderID = wo,
                                    isDispatchNotes = 0
                                };
                                cse.NotesHistories.Add(notesHistory);

                                entity.SaveChanges();

                                result.WorkOrderId = cse.WorkOrderID;
                                result.WorkorderStatus = cse.NonServiceEventStatus;
                                result.CloseStatus = "Success";
                            }
                            catch (Exception ex)
                            {
                                result.WorkOrderId = cse.WorkOrderID;
                    ...
                    {
                        WorkOrder workOrder = entity.WorkOrders.Where(w => w.WorkorderID == wo).FirstOrDefault();
                        if (workOrder == null)
                        {
                            result.WorkOrderId = wo;
                            result.WorkorderStatus = "";
                            result.CloseStatus = "Failed";
                            result.Message = "Invalid Workorder Id";
                        }
                        else
                        {
                            try
                            {
                                DateTime CurrentTime = Utility.GetCurrentTime(workOrder.CustomerZipCode, entity);

                                workOrder.WorkorderCallstatus = "Closed";
                                workOrder.NoServiceRequired = true;
                                workOrder.WorkorderCloseDate = CurrentTime;
                                workOrder.WorkorderCloseUserid = System.Web.HttpContext.Current.Session["UserId"] != null ? Convert.ToInt32(System.Web.HttpContext.Current.Session["UserId"]) : 1234;

                                NotesHistory notesHistory = new NotesHistory()
                                {
                                    AutomaticNotes = 1,
                                    EntryDate = CurrentTime,
                                    Notes = "Workorder Closed as NSR as per the ESM",
                                    Userid = System.Web.HttpContext.Current.Session["UserId"] != null ? Convert.ToInt32(System.Web.HttpContext.Current.Session["UserId"]) : 1234,
                                    UserName = UserName,
                                    WorkorderID = wo,
                                    isDispatchNotes = 0
                                ...
                                result.WorkorderStatus = workOrder.WorkorderCallstatus;
                                result.CloseStatus = "Success";
                            }
                            catch (Exception ex)
                            {
                ...
                /*
                 List<WorkOrder> WorkordersList = entity.WorkOrders.Where(w => eventsList.Contains(w.WorkorderID) && w.WorkorderCallstatus != "Closed").ToList();
                foreach(WorkOrder wo in WorkordersList)
                {
                    WorkorderNSRCloseResultModel result = new WorkorderNSRCloseResultModel();
                    try
                    {
                        DateTime CurrentTime = Utility.GetCurrentTime(wo.CustomerZipCode, entity);

                        wo.WorkorderCallstatus = "Closed";
                        wo.NoServiceRequired = true;
                        wo.WorkorderCloseDate = CurrentTime;
                        wo.WorkorderCloseUserid = System.Web.HttpContext.Current.Session["UserId"] != null ? Convert.ToInt32(System.Web.HttpContext.Current.Session["UserId"]) : 1234;
                        ...
                        result.WorkOrderId = wo.WorkorderID;
                        result.WorkorderStatus = wo.WorkorderCallstatus;
                        result.CloseStatus = "Success";
                    }
                    catch(Exception ex)
                    {
            ...
            JsonResult jsonResult = new JsonResult();
            jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, data = resultList, message = "" };
            jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
            return jsonResult;
        }

        public ActionResult ExtractWorkorderIds(HttpPostedFileBase file)
        {
            try
            {
                string workorderIdsList = "";
                if (file == null)
                {
                    ViewBag.Message = "No File Selected ";
                    ViewBag.isSuccess = false;
                    ViewBag.dataSource = "";
                    return View("WorkorderNSRClose");
                }
                ...
                {
                    string _FileName = Path.GetFileName(file.FileName);
                    string DirPath = Server.MapPath("~/UploadedFiles/CloseWorkorders");
                    DateTime currentDate = DateTime.Now;
                    if (!Directory.Exists(DirPath))
                    {
                        Directory.CreateDirectory(DirPath);
                    ...
                    while ((line = csvReader.ReadLine()) != null)
                    {
                        ...
                        if (i != 0)
                        {
                            ...
                            for (int ind = 0; ind <= lineValues.Count() - 1; ind++)
                            {
                                ...
                                {
                                    case 0:
                                        workorderIdsList += string.IsNullOrEmpty(str) ? "" : str.Trim();
                                        workorderIdsList += ",";
                                        //if (string.IsNullOrEmpty(CustomerName)) { ErrorMessage += "WorkorderId is Missing"; }
                                        break;
                                }
                            }
                ...
                ViewBag.dataSource = workorderIdsList.Trim();
                return View("WorkorderNSRClose");
            }
            catch (Exception ex)
            {
