                ...
                case ERFManagementSubmitType.SAVE:
                    {
                        ...
                        else
                        {
                            ...
                            foreach (ERFManagementEquipmentModel equipment in erfModel.ErfAssetsModel.EquipmentList)
                            {
                                if (equipment.Quantity.HasValue == false || equipment.Quantity == 0)
                                {
                                    message += @"|Quantity is required for equipment at row " + nCount;
                                    isValid = false;
                                }
                                if (equipment.Category.HasValue == false || equipment.Category == 0)
                                {
                                    message += @"|Category is required for equipment at row " + nCount;
                                    isValid = false;
                                }
                                if (equipment.Brand.HasValue == false || equipment.Brand == 0)
                                {
                                    message += @"|Brand Type is required for equipment at row " + nCount;
                                    isValid = false;
                                }
                                else
                                {
                                    ContingentDetail contingentDtl = FarmerBrothersEntitites.ContingentDetails.Where(c => c.ContingentID == equipment.Category && c.ID == equipment.Brand).FirstOrDefault();
                                    if (contingentDtl == null)
                                    {
                                        message += @"|Category and Brand Type does not match for equipment at row " + nCount;
                                        isValid = false;
                                    }
                                }

                                if (string.IsNullOrWhiteSpace(equipment.Branch))
                                {
                                    message += @"|Branch is required for equipment at row " + nCount;
                                    isValid = false;
                                }
                                if (string.IsNullOrWhiteSpace(equipment.Substitution))
                                {
                                    message += @"|Substitution is required for equipment at row " + nCount;
                                    isValid = false;
                                }
                                if (string.IsNullOrWhiteSpace(equipment.TransactionType))
                                {
                                    message += @"|TransactionType is required for equipment at row " + nCount;
                                    isValid = false;
                                }
                                if (string.IsNullOrWhiteSpace(equipment.EquipmentType))
                                {
                                    message += @"|EquipmentType is required for equipment at row " + nCount;
                                    isValid = false;
                                }
                                nCount++;
                            }
                            ...
                            nCount = 0;
                            foreach (ERFManagementExpendableModel expendable in erfModel.ErfAssetsModel.ExpendableList)
                            {
                                if (expendable.Quantity.HasValue == false || expendable.Quantity == 0)
                                {
                                    message += @"|Quantity is required for expendable at row " + nCount;
                                    isValid = false;
                                }
                                if (expendable.Category.HasValue == false || expendable.Category == 0)
                                {
                                    message += @"|Category is required for expendable at row " + nCount;
                                    isValid = false;
                                }
                                if (expendable.Brand.HasValue == false || expendable.Brand == 0)
                                {
                                    message += @"|Brand Type is required for expendable at row " + nCount;
                                    isValid = false;
                                }
                                else
                                {
                                    ContingentDetail contingentDtl = FarmerBrothersEntitites.ContingentDetails.Where(c => c.ContingentID == expendable.Category && c.ID == expendable.Brand).FirstOrDefault();
                                    if (contingentDtl == null)
                                    {
                                        message += @"|Category and Brand Type does not match for expendable at row " + nCount;
                                        isValid = false;
                                    }
                                }

                                if (string.IsNullOrWhiteSpace(expendable.Branch))
                                {
                                    message += @"|Branch is required for expendable at row " + nCount;
                                    isValid = false;
                                }
                                if (string.IsNullOrWhiteSpace(expendable.Substitution))
                                {
                                    message += @"|Substitution is required for expendable at row " + nCount;
                                    isValid = false;
                                }
                                if (string.IsNullOrWhiteSpace(expendable.TransactionType))
                                {
                                    message += @"|TransactionType is required for expendable at row " + nCount;
                                    isValid = false;
                                }
                                if (string.IsNullOrWhiteSpace(expendable.EquipmentType))
                                {
                                    message += @"|EquipmentType is required for expendable at row " + nCount;
                                    isValid = false;
                                }
                                nCount++;
                            }
        ...
        public bool ERFEmail(string ErfId, int? workorderId, bool isAccessory, string approvalStatus, bool isCancel = false, bool isFromApprove = false)
        {
            ...
            if (erfData != null)
            {
                ...
                subject.Append(subjectLine);
                    ...
                    message.Body = salesEmailBody.Replace("cid:logo", " ").ToString();
                ...
                if (!string.IsNullOrWhiteSpace(mailTo))
                {
                    ...
                    message.From = new MailAddress(fromAddress);
                        ...
                        message.IsBodyHtml = true;
                    message.ReplyToList.Add(new MailAddress(ConfigurationManager.AppSettings["ERFMailReplyToAddress"]));

                    //bool result = true;
                    try
                    {
                        EmailUtility eu = new EmailUtility();
                        eu.SendEmail(fromAddress, mailTo, mailCC, subject.ToString(), salesEmailBody.ToString(), mailBcc);
                    }
                    catch
                    {
                        result = false;
