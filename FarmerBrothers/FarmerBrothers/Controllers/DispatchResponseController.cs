...
namespace FarmerBrothersMailResponse.Controllers
{
    public class DispatchResponseController : BaseController
    {
        ...
        public ActionResult DispatchResponse(int workOrderId, int techId, DispatchResponse response, bool isResponsible, string isBillable = "False")
        {
            ...
            if(workOrder != null && workOrder.WorkorderCallstatus == "Closed")
            {
                dispatchModel.Message = "Workorder is already Closed";
                return View("DispatchResponse", "_Layout_WithOutMenu", dispatchModel);
            }

            WorkorderSchedule techWorkOrderSchedule = workOrder.WorkorderSchedules.Where(ws => ws.Techid == techId).FirstOrDefault();
            if(techWorkOrderSchedule == null && response != FarmerBrothers.Data.DispatchResponse.REDIRECTED)
            {
            ...
            if (techId != 0 && techHierarchyView != null)
            {
                if (workOrder != null)
                {
                    ...
                    switch (response)
                    {
                        case FarmerBrothers.Data.DispatchResponse.ACCEPTED:
                            //updatedStatus = UpdateTechAssignedStatus(techId, workOrder, "Accepted", false);
                            if (UpdateTechAssignedStatus(techId, workOrder, "Accepted", false))
                            {
                                CopyWorkorderEquipments(workOrder);
                                workOrder.IsRejected = false;
                        ...
                        case FarmerBrothers.Data.DispatchResponse.REJECTED:
                            ...
                            ViewBag.isERF = false;
                            ViewBag.text = "Rejected";

                            return View("ConfirmationWindow", ViewBag);

                            //if (UpdateTechAssignedStatus(techId, workOrder, "Declined", false))
                            //{
                            //    workOrder.IsRejected = false;
                            //    DateTime currentTime = Utility.GetCurrentTime(workOrder.CustomerZipCode, FarmerBrothersEntitites);
                            //    workOrder.WorkorderModifiedDate = currentTime;
                        ...
                        case FarmerBrothers.Data.DispatchResponse.STARTED:
                            ...
                            else
                            {
                                if (workOrder != null
                                    && (string.Compare(workOrder.WorkorderCallstatus, "Accepted", true) == 0
                                        || string.Compare(workOrder.WorkorderCallstatus, "Accepted-Partial", true) == 0
                                        || string.Compare(workOrder.WorkorderCallstatus, "Scheduled", true) == 0))
                                {
                                    ViewBag.workOrderId = workOrderId;
                                    ViewBag.techId = techId;
                                    ViewBag.response = response;
                                    ...
                                    //        details.ResponsibleTechName = techHierarchyView.PreferredProvider;
                                    //    }
                                    //    if (string.Compare(workOrder.WorkorderCallstatus, "Scheduled", true) == 0)
                                    //    {
                                    //        workOrder.WorkorderCallstatus = "Accepted";
                                    //        techWorkOrderSchedule.AssignedStatus = "Accepted";
                                    //    }
                                ...
                                else
                                {
                                    ...
                                    {
                                        //dispatchModel.Message = "Work Order is in '" + workOrder.WorkorderCallstatus + "' Status, Update Start Status only if the WorkOrder Status is in 'Accepted' or 'Accepted-Partial' ";
                                        dispatchModel.Message = "Work Order is in <b> '" + workOrder.WorkorderCallstatus + "' </b> Status <br/> 'Start' Time can be changed only if the Work order is in <b> 'Accepted' or 'Accepted-Partial' or 'Scheduled' </b> Status.";
                                    }
                                    //dispatchModel.TechId = techId;
                                    //dispatchModel.WorkOrderId = workOrderId;
                                    //dispatchModel.Message = "Work Order is not yet Accepted!";
        ...
        [AllowAnonymous]
        public JsonResult UpdateEventTimeStamps(int workOrderId, int techId, DispatchResponse response, bool isResponsible)
        {
            JsonResult jsonResult = new JsonResult();
            ...
            {
                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "No schedule found, Please resend the email and try again" };
                jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
                return jsonResult;
            }
            ...
            if (techId != 0 && techHierarchyView != null)
            {
                if (workOrder != null)
                {
                    ...
                    switch (response)
                    {
                        case FarmerBrothers.Data.DispatchResponse.STARTED:
                            if (techWorkOrderSchedule != null && techWorkOrderSchedule.AssignedStatus == "Declined")
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Cannot Start, You Rejected the Call" };
                            }
                            else if (techWorkOrderSchedule != null && techWorkOrderSchedule.AssignedStatus == "Redirected")
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Cannot Operate Event, Call Has been Redirected by You" };                                
                            }
                            else
                            {
                                if (workOrder != null
                                        ...
                                        || string.Compare(workOrder.WorkorderCallstatus, "Scheduled", true) == 0))
                                {
                                    ...
                                    if (workOrder.WorkorderDetails != null && workOrder.WorkorderDetails.Count > 0)
                                    {
                                        ...
                                        if (FarmerBrothersEntitites.SaveChanges() > 0)
                                        {
                                            NonFBCustomer nfbcust = (from ncust in FarmerBrothersEntitites.NonFBCustomers
                                                                     join c in FarmerBrothersEntitites.Contacts on ncust.NonFBCustomerId equals c.PricingParentID
                                                                     where c.ContactID == workOrder.CustomerID
                                                                     select ncust).FirstOrDefault();

                                            if (nfbcust != null && nfbcust.EventScheduleEmail != null)
                                            {
                                                SendEventTimestampEmail(workOrderId, "Start", currentTime, nfbcust.EventScheduleEmail);
                                            }

                                            jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, data = currentTime.ToString(), workorderid = workOrderId, message = "Started status updated!" };
                                        }
                                    }
                                    else
                                    {
                                        jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "There is a problem in updating status! Please contact support!" };
                                    }
                                }
                                else
                                {
                                    ...
                                    if (workOrder == null)
                                    {
                                        jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Error Fetching the Workorder, Try Again" };
                                    }
                                    else if (string.Compare(workOrder.WorkorderCallstatus, "Accepted", true) != 0
                                        || string.Compare(workOrder.WorkorderCallstatus, "Accepted-Partial", true) != 0)
                                    {
                                        string Message = "Work Order is in '" + workOrder.WorkorderCallstatus + "' Status 'Start' Time can be changed only if the Work order is in 'Accepted' or 'Accepted-Partial' or 'Scheduled' Status.";
                                        jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = Message };
                                    }
                                }
                            }
                            break;
                        case FarmerBrothers.Data.DispatchResponse.ARRIEVED:
                            if (techWorkOrderSchedule != null && techWorkOrderSchedule.AssignedStatus == "Declined")
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Cannot update Arrival status, You Rejected the Call" };
                            }
                            else if (techWorkOrderSchedule != null && techWorkOrderSchedule.AssignedStatus == "Redirected")
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Cannot Operate Event, Call Has been Redirected by You" };                                
                            ...
                            else
                            {
                                ...
                                if (workOrder != null && workOrder.WorkorderDetails != null && workOrder.WorkorderDetails.Count > 0)
                                {
                                    ...
                                    if (details.StartDateTime != null)
                                    {
                                        ...
                                        if (diff.TotalHours > 4)
                                        {
                                            jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "ArrivalDate cannot be more than 4 Hours from Start Time. Please update the Arrival Date from Closure applicaiton" };
                                        }
                                        else
                                        {
                                            if (workOrder != null
                                                ...
                                                || string.Compare(workOrder.WorkorderCallstatus, "Accepted-Partial", true) == 0))
                                            {
                                                ...
                                                if (workOrder.WorkorderDetails != null && workOrder.WorkorderDetails.Count > 0)
                                                {
                                                    ...
                                                    if (FarmerBrothersEntitites.SaveChanges() > 0)
                                                    {
                                                        ...
                                                        if (nfbcust != null && nfbcust.EventScheduleEmail != null)
                                                        {
                                                            SendEventTimestampEmail(workOrderId, "Arrival", currentTime, nfbcust.EventScheduleEmail);
                                                        }

                                                        jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, data = currentTime.ToString(), workorderid = workOrderId, message = "Arrival status updated!" };
                                                    }
                                                }
                                                else
                                                {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Work Order is already in 'On Site' Status " };
                                                }
                                                else if (string.Compare(workOrder.WorkorderCallstatus, "Accepted", true) != 0
                                                    || string.Compare(workOrder.WorkorderCallstatus, "Accepted-Partial", true) != 0)
                                                {
                                                    string Message = "Work Order is in '" + workOrder.WorkorderCallstatus + "' Status 'Arrival/OnSIte' Status can be changed only if the Work order is in 'Accepted'  or  'Accepted-Partial' Status.";
                                                    jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = Message };
                                                }
                                    ...
                                    {
                                        jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "StartDate is not updated. Please set the StartDate before ArrivalDate" };
                                    }
                                }
                            }
                            break;
                        case FarmerBrothers.Data.DispatchResponse.REDIRECTED:
                            if (techWorkOrderSchedule != null && techWorkOrderSchedule.AssignedStatus == "Declined")
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Cannot Redirect, You Rejected the Call" };
                            }
                            else if (techWorkOrderSchedule != null && (techWorkOrderSchedule.AssignedStatus == "Sent" || techWorkOrderSchedule.AssignedStatus == "Accepted"))
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Call has been Sent/Accepted by the selected Tech" };                                
                            }
                            else
                            {
                                //WorkorderSchedule ws = FarmerBrothersEntitites.WorkorderSchedules.Where(sc => sc.Techid == techId && (sc.AssignedStatus.ToLower() == "sent" || sc.AssignedStatus.ToLower() == "accepted")).FirstOrDefault();
                                ...
                                if (ws == null)
                                {
                                    if (workOrder.WorkorderCallstatus == "Closed")
                                    {
                                        jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Work Order can't be redirected!" };
                                        break;
                                    }
                                    ...
                                    if (FarmerBrothersEntitites.SaveChanges() > 0)
                                    {
                                        jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, data = true, workorderid = workOrderId, message = "Work Order has been redirected successfully!" };
                                    }
                                }
                                else
                                {
                                    jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Work Order can't be redirected! It is Assigned to Third Party Tech" };
                                }
                            }
                            break;
                        ...
                        case FarmerBrothers.Data.DispatchResponse.COMPLETED:
                            if (techWorkOrderSchedule != null && techWorkOrderSchedule.AssignedStatus == "Declined")
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Cannot Complete, You Rejected the Call" };
                            }
                            else if (techWorkOrderSchedule != null && techWorkOrderSchedule.AssignedStatus == "Redirected")
                            {
                                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Cannot Operate Event, Call Has been Redirected by You" };
                            ...
                            else
                            {
                                ...
                                if (workOrder != null && workOrder.WorkorderDetails != null && workOrder.WorkorderDetails.Count > 0)
                                {
                                    ...
                                    if (details.ArrivalDateTime != null)
                                    {
                                        ...
                                        if (diff.TotalHours > 12)
                                        {
                                            jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "CompletionDate cannot be more than 12 Hours from Arrival Time. Please update the Completion Date from Closure applicaiton" };
                                        }
                                        else
                                        {
                                            if (workOrder != null
                                                && string.Compare(workOrder.WorkorderCallstatus, "On Site", true) == 0)
                                            {
                                                ...
                                                if (FarmerBrothersEntitites.SaveChanges() > 0)
                                                {
                                                    ...
                                                    if (nfbcust != null && nfbcust.EventScheduleEmail != null)
                                                    {
                                                        SendEventTimestampEmail(workOrderId, "Complete", currentTime, nfbcust.EventScheduleEmail);
                                                    }

                                                    jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, data = currentTime.ToString(), workorderid = workOrderId, message = "Work Order completed successfully!" };
                                                }
                                            }
                                            else
                                            {
                                                ...
                                                {
                                                    jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Work Order is already in 'Completed' Status " };
                                                }
                                                else if (string.Compare(workOrder.WorkorderCallstatus, "On Site", true) != 0)
                                                {
                                                    string Message = "Work Order is in <b> '" + workOrder.WorkorderCallstatus + "' Status 'Completed' Status can be changed only if the Work order is in 'On site' Status.";
                                                    jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = Message };
                                                }
                                            }
                                        }
                                    ...
                                    {
                                        jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "ArrivalDate is not updated. Please set the ArrivalDate before CompletionDate" };
                                    }
                                }
                            }
                            break;
                        ...
                        //        if (workOrder.WorkorderCallstatus.ToLower() != "completed" && workOrder.WorkorderCallstatus.ToLower() != "closed" && workOrder.WorkorderCallstatus.ToLower() != "invoiced")
                        //        {
                        //            RedirectToScheduleView(workOrder.CustomerID, workOrder.WorkorderID, techId);
                        //        }
                        //        else
                        //        {
                        //            dispatchModel.Message = "Work Order is Already Closed/Completed";
                ...
                {
                    jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "There is an error in fetching the Work Order! Please contact support!" };
                    jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
                    return jsonResult;
                }
            }
        ...
        [AllowAnonymous]
        public JsonResult RejectEvent(int workOrderId, int techId, DispatchResponse response, bool isResponsible)
        {
            JsonResult jsonResult = new JsonResult();
            WorkOrder workOrder = FarmerBrothersEntitites.WorkOrders.Where(w => w.WorkorderID == workOrderId).FirstOrDefault();
            if (UpdateTechAssignedStatus(techId, workOrder, "Declined", false))
            {
                ...
                if (FarmerBrothersEntitites.SaveChanges() > 0)
                {
                    jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, data = true, workorderid = workOrderId, message = "Work Order rejected!" };                    
                }
                else
                {
                    jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "There is an error in declining the Work Order! Please contact support!" };
                }
            }
            else
            {
                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = true, workorderid = workOrderId, message = "Work Order has been redirected! You need not reject the Work Order!" };
            }
        ...
        //private ActionResult RedirectToScheduleView(int? CustomerId, int WorkorderId, int TechId)
        //{
        //    return RedirectToActionPermanent("ScheduleEvent", "ScheduleEvent", new { customerId = CustomerId, workOrderId = WorkorderId, techId = TechId });
        //}
        ...
        [AllowAnonymous]
        public ActionResult ERFStatusUpdate_Old(string ERFId, string ESM, string Status)
        {
            DispatchResponseModel dispatchModel = new DispatchResponseModel();
        ...
        {
            DispatchResponseModel dispatchModel = new DispatchResponseModel();
            using (FarmerBrothersEntities context = new FarmerBrothersEntities())
            {
                Erf erf = context.Erfs.Where(er => er.ErfID == ERFId).FirstOrDefault();
                if (Status.ToLower() == "cancel")
                {
                    if (erf.ERFStatus.ToLower() == "cancel")
                    {
                ...
                DateTime CurrentTime = Utility.GetCurrentTime(erf.CustomerZipCode, context);
                int esmId = Convert.ToInt32(ESM);
                
                ESMCCMRSMEscalation esmdsmrsmView = context.ESMCCMRSMEscalations.FirstOrDefault(x => x.EDSMID == esmId);
                NotesHistory notesHistory = new NotesHistory()
                {
                    AutomaticNotes = 1,
                    EntryDate = CurrentTime,
        ...
        public bool SendEventTimestampEmail(int WorkorderId, string Status, DateTime TimeStamp, string ToEmail)
        {
            bool result = true;
            StringBuilder salesEmailBody = new StringBuilder();
            StringBuilder subject = new StringBuilder();
            ...
            subject.Append("Event Status Email - WO: ");
            subject.Append(workOrder.WorkorderID);
            subject.Append(" Status: ");
            subject.Append(workOrder.WorkorderCallstatus);
            ...
            {
                salesEmailBody.Append("Technician Started To Location at : ");
            }
            else if (Status.ToUpper() == "ARRIVAL")
            {
                salesEmailBody.Append("Technician Arrived at the location at : ");
            }
            if (Status.ToUpper() == "COMPLETE")
            {
                salesEmailBody.Append("Technician Completed the Event at : ");
            }

            salesEmailBody.Append(TimeStamp);
            salesEmailBody.Append("</b></span>");


            salesEmailBody.Append("<BR>");
            salesEmailBody.Append("Technician: ");
            salesEmailBody.Append(workOrder.ResponsibleTechName);
            salesEmailBody.Append("<BR>");
            salesEmailBody.Append("<BR>");
