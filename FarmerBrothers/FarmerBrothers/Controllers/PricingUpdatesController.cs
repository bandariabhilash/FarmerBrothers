...
namespace FarmerBrothers.Controllers
{
    public class PricingUpdatesController : BaseController
    {
        ...
        public ActionResult PricingUpdates()
        {
            ...
            var extraParentDetails = FarmerBrothersEntitites.PricingDetails.Where(p => p.PricingTypeId == 501 && p.IsNonFBParent == false).ToList();
            foreach(var parent in extraParentDetails)
            {
                if (string.IsNullOrWhiteSpace(parent.PricingEntityName)) continue;

                model.ParentDetails.Add(new
                NonFBCustomer{
                    IsActive = true,
                    NonFBCustomerId = parent.PricingEntityId,
                    NonFBCustomerName = parent.PricingEntityName                    
                });
            }
        ...
        [HttpGet]
        public JsonResult GetNonFBCustomerDetails(string name)
        {
            JsonResult jsonResult = new JsonResult();
            try
            {
                NonFBCustomer nfbc = Utility.GetNonFBCustomer(name, FarmerBrothersEntitites);
                if(nfbc == null)
                {
                    PricingDetail pdtls = FarmerBrothersEntitites.PricingDetails.Where(p => p.PricingEntityName.ToLower() == name.ToLower()).FirstOrDefault();
                    if (pdtls != null)
                    {
                        nfbc = new NonFBCustomer();
                        nfbc.NonFBCustomerId = pdtls.PricingEntityId;
                        nfbc.NonFBCustomerName = pdtls.PricingEntityName;
                    }
                }

                jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, data = nfbc };
                jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
            }
            catch(Exception ex)
            {
                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, data = "" };
                jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
            }
        ...
        public List<PriceDataModel> GetParentPricingDetails_old()
        {
            ...
            List<NonFBCustomer> parentList = FarmerBrothersEntitites.NonFBCustomers.Where(n => n.IsActive == true).ToList();
        ...
        public List<PriceDataModel> GetParentPricingDetails()
        {
            ...
            var PricingDataList = (from pric in FarmerBrothersEntitites.PricingDetails
                                   where pric.PricingTypeId == pTypeId
                                   select new
                                   {
                                       id = pric.Id,
                                       parentId = pric.PricingEntityId,
                                       parentName = pric.PricingEntityName,
                                       hourlyLaborPrice = pric.HourlyLablrRate == null ? 0 : pric.HourlyLablrRate,
                                       hourlyTravelPrice = pric.HourlyTravlRate == null ? 0 : pric.HourlyTravlRate,
                                       afterHourLaborPrice = pric.AfterHoursLaborRate == null ? 0 : pric.AfterHoursLaborRate,
                                       afterHourTravlePrice = pric.AfterHoursTravelRate == null ? 0 : pric.AfterHoursTravelRate,
                                       ...
                                       partsDiscount = pric.PartsDiscount == null ? 0 : pric.PartsDiscount,
                                       approved3rdPartyUse = pric.Approved3rdPartyUse,
                                       isNonFBParent = pric.IsNonFBParent
                                   }).ToList();
            foreach (var priceItem in PricingDataList)
            {
                PriceDataModel data = new PriceDataModel();
                ...
                data.AfterHourRatesApply = Convert.ToBoolean(priceItem.afterHourRateApply);
                data.Approved3rdPartyUse = Convert.ToBoolean(priceItem.approved3rdPartyUse);
                data.IsNonFBParent = Convert.ToBoolean(priceItem.isNonFBParent);
        ...
        public ActionResult ParentPricingUpdate_old(PriceDataModel value)
        {
        ...
        public ActionResult ParentPricingUpdate(PriceDataModel value)
        {
            ...
            NonFBCustomer fbCust = FarmerBrothersEntitites.NonFBCustomers.Where(c => c.NonFBCustomerId == value.PricingEntityId).FirstOrDefault();
                ...
                if (fbCust != null)
                {
                    ...
                    if (ParentItem != null)
                    {
                        ...
                        ParentItem.PartsDiscount = value.PartsDiscount;
                        ParentItem.Approved3rdPartyUse = value.Approved3rdPartyUse;
                        ParentItem.IsNonFBParent = value.IsNonFBParent;
                    }
                    else
                    {
                        PricingDetail ParentItm = new PricingDetail();
                        ...
                        ParentItm.PricingTypeId = FarmerBrothersEntitites.PricingTypes.Where(p => p.PricingTypeName.ToLower() == "parent").Select(s => s.PricingTypeId).FirstOrDefault();
                        ParentItm.Approved3rdPartyUse = value.Approved3rdPartyUse;
                        ParentItm.IsNonFBParent = value.IsNonFBParent;
            ...
            {
                PricingDetail ParentItem = FarmerBrothersEntitites.PricingDetails.Where(p => p.PricingEntityId.ToString() == ParentId.ToString()).FirstOrDefault();
                ...
                else
                {
                    ...
                    ParentItm.PricingEntityId = ParentId.ToString();
        ...
        public List<PriceDataModel> Get3rdPartyPricingDetails()
        {
            ...
            var PricingDataList = (from parent in FarmerBrothersEntitites.TECH_HIERARCHY
                                   ...
                                   from pric in pricingData.DefaultIfEmpty()
                                   where parent.FamilyAff.ToLower() == "spt" && parent.SearchType.ToLower() != "spi"
                                   select new
                                   {
                                       //id = pric.Id,
