...
namespace FarmerBrothers.Controllers
{
    public class CallClosureController : BaseController
    {
        ...
        WorkorderManagementModel ConstructWorkorderManagementModel(int? customerId, int? workOrderId, bool isFromProcessCardScreen = false)
        {
            ...
            workOrderManagementModel.CallTypes = FarmerBrothersEntitites.WorkorderTypes.Where(wt => wt.ViewIn.ToLower() == "mars" && wt.Active == 1).OrderBy(wt => wt.Sequence).ToList();
            workOrderManagementModel.ClosureCallTypes = FarmerBrothersEntitites.WorkorderTypes.Where(wt => wt.ViewIn.ToLower() == "mars" && wt.Active == 1).OrderBy(wt => wt.Sequence).ToList();
            ...
            if (workOrderManagementModel.WorkOrder != null)
            {
                ...
                if (ws != null)
                {
                    ...
                    workOrderManagementModel.ScheduleUrl = string.Format("{0}{1}&encrypt=yes", url, new Encrypt_Decrypt().Encrypt("workOrderId=" + workOrderManagementModel.WorkOrder.WorkorderID + "&techId=" + ws.Techid + "&response=8&isResponsible=true"));
                }
        ...
        public JsonResult SaveWorkOrder([ModelBinder(typeof(CallCloserModelBinder))] WorkorderManagementModel workorderManagement)
        {
            ...
            switch (workorderManagement.Operation)
            {
                case WorkOrderManagementSubmitType.NOTIFYSALES:
                    returnValue = wc.NotifySales(workorderManagement, out workOrder);
                    if (returnValue > 0)
                    {
                        jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, Url = "", WorkOrderId = workOrder.WorkorderID, returnValue = 1, WorkorderCallstatus = workOrder.WorkorderCallstatus, message = message };
                        jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
                        return jsonResult;
                    }
                    else
                    {
                        jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, Url = "", WorkOrderId = 0, returnValue = returnValue, WorkorderCallstatus = "", message = message };
                        jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
                        return jsonResult;
                    }
                    break;
                case WorkOrderManagementSubmitType.OVERTIMEREQUEST:
                    returnValue = wc.OverTimeRequest(workorderManagement, out workOrder);
                    ...
                    break;
                default:
                    returnValue = CloseWorkOrder(workorderManagement, out message, out workOrder);
                    ...
                    if (string.IsNullOrEmpty(message) || message.Contains("Spawned Work Order"))
                    {
                        ...
                        {
                            message = "Workorder : " + workOrder.WorkorderID + " Saved Successfully";
                            jsonResult.Data = new { success = true, serverError = ErrorCode.SUCCESS, Url = redirectUrl, WorkOrderId = workOrder.WorkorderID, returnValue = 1, WorkorderCallstatus = workOrder.WorkorderCallstatus, message = message};
                        }
                        jsonResult.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
                        return jsonResult;
                    }
                ...
                using (FarmerBrothersEntities newEntity = new FarmerBrothersEntities())
                {
                    using (System.Data.Entity.DbContextTransaction dbTran = newEntity.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            SaveNotes(workorderManagement, workOrder, newEntity);
                            ...
                            if (workorderManagement.WorkOrder.WorkorderID > 0 && workorderManagement.Operation == WorkOrderManagementSubmitType.COMPLETE)
                            {
        ...
        public void SaveNotes(WorkorderManagementModel workorderManagement, WorkOrder workOrder, FarmerBrothersEntities FarmerBrothersEntitites)
        {
