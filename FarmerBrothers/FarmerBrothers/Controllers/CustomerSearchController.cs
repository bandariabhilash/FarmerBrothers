...
namespace FarmerBrothers.Controllers
{
    public class CustomerSearchController : BaseController
    {
                ...
                try
                {
                    if (customerSearchModel != null)
                    {
                        try
                        {
                            ...
                            var contacts = FarmerBrothersEntitites.Set<Contact>().AsExpandable().Where(predicate).Take(100); 
                            ...
                            foreach (Customer customer in contacts)
                            {
                               ...
                               (Dictionary<string, string>)System.Web.HttpContext.Current.Session["UserPrivilege" + (int)System.Web.HttpContext.Current.Session["UserId"]];
                                ...
                                {
                                    customer.OnHold = nonFBCustomer == null ? false : (nonFBCustomer.OnHold == null ? false : nonFBCustomer.OnHold);
                                }
        ...
        public bool isActiveNonFBCustomer(string nonFBCustomerId)
        {
            bool? custActive = FarmerBrothersEntitites.NonFBCustomers.First(c => c.NonFBCustomerId == nonFBCustomerId).IsActive;
            return custActive == null ? true : Convert.ToBoolean(custActive);
        }
