...
namespace FarmerBrothers.Utilities
{
    public class EmailUtility
    {
        ...
        public string SendEmail(string fromAddress, string toAddress, string CcAddress, string subject, string message, string bccAddress = "")
        {
            try
            {
                ...
                {
                    string[] bccMail = bccAddress.Split(';');
                    int j = 0;
                    for (j = 0; j < bccMail.Count(); j++)
                    {
                        if (string.IsNullOrEmpty(bccMail[j])) continue;
                ...
                {
                    mailMessage.BccRecipients = bccRecipients;
                }
